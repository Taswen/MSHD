...
def timestr2timecode(timestr:str):
    ## 2021-04-21 10:15:57
    p = re.compile(r"(?P<year>\d{,4})[-/](?P<mouth>\d{,2})[-/](?P<day>\d{,2})\s+(?P<hour>\d{,2}):(?P<minute>\d{,2}):(?P<secound>\d{,2})")
    res = p.match(timestr)
...
def eqEncode(country,posotion,time,level):
    return (country2countrycode(country)+LAL2Positioncode(*posotion)+timestr2timecode(time)+level2levelcode(level))


if __name__ == "__main__":
    print(eqEncode("中国",(10.2,5.5),"2021-04-21 10:15:57",3.4))
